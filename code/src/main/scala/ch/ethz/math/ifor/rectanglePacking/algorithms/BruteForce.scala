...
object BruteForce extends Algorithm {
  ...
  /** Compute the  optimum for one tree
    ...
    * @return
    */
  def runAux(instance: Instance,possibleTops:List[Point],allAnchors:List[Anchor]):Map[Anchor,Rectangle] = {
    if (instance.anchors.length > 1) {
      runIt(instance, possibleTops, possibleTops,allAnchors )
    }
    else {
  ...
  /** For one anchor, goes through all the possible choices of topright for this anchor (breadth in the graph)
    ...
    * @return
    */
  def runIt(instance: Instance,choices:List[Point],possibleTops:List[Point],allAnchors:List[Anchor]):Map[Anchor,Rectangle]={
    ...
    if (choices.length>=1) {
      if (instance.anchors.head.dominatesStrict(choices.head) && allAnchors.forall(a=> !(a.inRectStrictUP(Rectangle(instance.anchors.head,choices.head)))) && instance.forbiddenRectangles.forall(r=> !(r.intersects(Rectangle(instance.anchors.head,choices.head)))) ) {
        ...
        val r1=runAux(newInst,possibleTops,allAnchors)
        val r2=runIt(instance,choices.tail,possibleTops,allAnchors)
        ...
        runIt(instance,choices.tail,possibleTops,allAnchors)
      }
    }
    else {
      ...
      runAux(newInst,possibleTops,allAnchors)+ (instance.anchors.head-> new Rectangle(instance.anchors.head,instance.anchors.head))
    }
  }
  ...
  def run(instance:Instance):Output=  {
    ...
    new Output(instance,runAux(instance,possibleTops,instance.anchors))
  }
