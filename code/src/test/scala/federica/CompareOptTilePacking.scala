...
object CompareOptTilePacking extends App {

  def runAllAlgorithms(instance: Instance, ratio: Double): Vector[String] = {
    ...
    if (Vector(outputBruteForce.objectiveValue/outputTilePacking1.objectiveValue,outputBruteForce.objectiveValue/outputTilePackingInf.objectiveValue).max > ratio) {
      ...
      val trace4: Vector[Scatter] = Html.scatterRectangles(outputTilePackingInf,"linfTilePacking",AxisReference.X3,AxisReference.Y3)
      val trace5: Vector[Scatter] = Html.scatterRectangles(outputBruteForce, "BruteForce", AxisReference.X4, AxisReference.Y4)

      // Subplot
      val subplot: Vector[Scatter] = trace1 ++ trace3 ++ trace4 ++ trace5
      subplot.plot(
        path = "outputFiles/htmlFiles/" + outputGreedy1.uuid + ".html",
        Layout(
          title = "OPT / Tilepacking greater than " + ratio,
          showlegend = false,
          height = 800,
          width = 800,
          xaxis = Axis(
      ...
      Vector(
        ...
        (outputBruteForce.objectiveValue/outputTilePacking1.objectiveValue).toString,
        (outputBruteForce.objectiveValue/outputTilePackingInf.objectiveValue).toString,
  ...
  val vectorNumberOfInstances: Vector[Integer] = Vector(10000,5000,1000,500,10,0)
  val ratio: Double = 1.2
  ...
  for (j <- 0 until numberOfValues){
    ...
    val data: Vector[Vector[String]] = instances.map(runAllAlgorithms(_,ratio)).filterNot(_==Vector()).map(i=>numberOfAnchors.toString +: i)
  ...
  def writeSpreadsheet(data:Vector[Vector[String]]): Unit = {
    ...
    val filename = System.getProperty("user.dir") + "/outputFiles/test/"+"CompareOptTile2.xlsx"
